...
import { findTextInDatabase } from '../../src/repositories/summary-repository';
...
describe('findTextInDatabase', () => {
  ...
  it('should find a summary with the given text', async () => {
    ...
    const summary = await findTextInDatabase(text);
  ...
  it('should return null if the text is not in the summaries collection', async () => {
    const summary = await findTextInDatabase('This text is not in the database.');
